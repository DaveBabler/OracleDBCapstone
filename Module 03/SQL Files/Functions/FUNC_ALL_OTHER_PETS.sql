...
AS
	...
	lv_reptilecount int :=0;
  	...
  	lv_loop int :=0; 
--the goal is to get ALL pets even previously dead ones so the counselor knows what the parent prefers in a pet
...
--and into a text format report
	...
	ELSE 
		...
		END IF;
		IF lv_felinecount > 0 
			THEN lv_other_pets := lv_other_pets || 'They own (or have owned) '||lv_felinecount||' cats of the following breeds: '||lv_feline||' '|| CHR(10);
		END IF;
